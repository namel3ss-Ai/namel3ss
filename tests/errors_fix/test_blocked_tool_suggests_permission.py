...
def test_blocked_tool_suggests_permission(tmp_path: Path) -> None:
    _write_fixture(tmp_path, "run", _load_fixture("run_last_tool.json"))
    _write_fixture(tmp_path, "tools", _load_fixture("tools_last_blocked.json"))
    ...
    assert "request_permission" in option_ids
