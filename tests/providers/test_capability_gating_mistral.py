...
def test_mistral_pipeline_skipped_when_capability_off(monkeypatch):
    ...
    monkeypatch.setattr("namel3ss.runtime.ai.providers.mistral.post_json", lambda **kwargs: {"choices": [{"message": {"content": "ok"}}]})
    result = execute_flow(
        program.flows[0],
