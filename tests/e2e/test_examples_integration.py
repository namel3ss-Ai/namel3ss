...
def test_examples_parse_and_manifest():
    for name in ["demo_crud_dashboard.ai", "demo_ai_assistant_over_records.ai", "demo_multi_agent_orchestration.ai"]:
...
def test_multi_agent_demo():
    source, ir_program = _load_program(EXAMPLES_DIR / "demo_multi_agent_orchestration.ai")
    state = {}
    store = MemoryStore()
