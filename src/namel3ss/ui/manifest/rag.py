...
def build_scope_selector_item(
    ...
    *,
    ...
    action_type, title, selection = _scope_selector_contract(item)
    ...
    if action_type != "scope_select":
        action["options_state"] = options_source
...
def _scope_selector_contract(item: ir.ScopeSelectorItem) -> tuple[str, str, str]:
    origin = getattr(item, "origin", None)
    if not isinstance(origin, dict):
        return ("scope_select", "Retrieval scope", "multi")
    rag_origin = origin.get("rag_ui")
    if not isinstance(rag_origin, dict):
        return ("scope_select", "Retrieval scope", "multi")
    binding = rag_origin.get("binding")
    selection = rag_origin.get("selection")
    selection_mode = str(selection).strip().lower() if isinstance(selection, str) else "multi"
    if selection_mode not in {"single", "multi"}:
        selection_mode = "multi"
    if binding == "threads":
        if selection_mode != "single":
            selection_mode = "single"
        return ("chat.thread.select", "Threads", selection_mode)
    if binding == "models":
        return ("chat.model.select", "Models", selection_mode)
    return ("scope_select", "Retrieval scope", selection_mode)
