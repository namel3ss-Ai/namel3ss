    ...
    for stmt in stmts:
        if isinstance(stmt, ast.AskAIStmt):
            ...
            for entry in stmt.entries:
                entry.agent_name = _resolve_name(
                    entry.agent_name,
                    kind="agent",
                    module_name=module_name,
                    alias_map=alias_map,
            ...
            stmt.record_name = _resolve_name(
                stmt.record_name,
                kind="record",
                module_name=module_name,
                alias_map=alias_map,
...
def _resolve_page_item(
    ...
    *,
    ...
    if isinstance(item, ast.FormItem):
        item.record_name = _resolve_name(
            item.record_name,
            kind="record",
            module_name=module_name,
            alias_map=alias_map,
    ...
    *,
    ...
    raise Namel3ssError(
        build_guidance_message(
            ...
            example=f'use "inventory" as inv',
        ),
        line=line,
        column=column,
    )
