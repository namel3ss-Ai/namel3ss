...
def parse_badge_item(parser, tok, *, allow_pattern_params: bool = False) -> ast.BadgeItem:
    ...
    if from_tok.type != "IDENT" or from_tok.value != "from":
        raise Namel3ssError(
            "Badge must use: badge from state.<path>",
            line=from_tok.line,
            column=from_tok.column,
        )
    parser._advance()
    # Allow optional legacy 'is' after from, mirroring other bindings.
    parser._match("IS")
    source = _parse_state_path_value_relaxed(parser, allow_pattern_params=allow_pattern_params)

    style = "neutral"
    if parser._current().type == "IDENT" and parser._current().value == "style":
        parser._advance()
        if parser._match("COLON"):
            pass
        else:
            parser._expect("IS", "Expected ':' or 'is' after style")
        style = _parse_ident_or_string(parser, context="badge style")
    if style not in _ALLOWED_BADGE_STYLES:
        raise Namel3ssError(
