...
from namel3ss.parser.records import type_from_token
    ...
    while parser._current().type != "DEDENT":
        ...
        if _match_word(parser, "implemented"):
            ...
            if not isinstance(kind_tok.value, str):
                raise Namel3ssError("Expected implementation kind after 'implemented using'", line=kind_tok.line, column=kind_tok.column)
            parser._advance()
        ...
        if tok.type == "TIMEOUT_SECONDS":
            ...
            if not is_number(value_tok.value):
                raise Namel3ssError("timeout_seconds must be a positive integer", line=value_tok.line, column=value_tok.column)
