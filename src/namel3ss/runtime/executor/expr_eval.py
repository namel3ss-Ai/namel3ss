...
def evaluate_expression(
    ...
    if isinstance(expr, ir.UnaryOp):
        operand = evaluate_expression(ctx, expr.operand, collector)
        if expr.op == "not":
    ...
    if isinstance(expr, ir.BinaryOp):
        if expr.op == "and":
            left = evaluate_expression(ctx, expr.left, collector)
            ...
            if not left:
                ...
                return False
            right = evaluate_expression(ctx, expr.right, collector)
    ...
    if isinstance(expr, ir.ListOpExpr):
        target = evaluate_expression(ctx, expr.target, collector)
        if expr.kind == "length":
        ...
        if expr.kind == "append":
            ...
            value = evaluate_expression(ctx, expr.value, collector)
        ...
        if expr.kind == "get":
            ...
            index = evaluate_expression(ctx, expr.index, collector)
    ...
    if isinstance(expr, ir.ListMapExpr):
        ...
        try:
            for idx, item in enumerate(target):
                ...
                result_item = evaluate_expression(ctx, expr.body, None)
    ...
    if isinstance(expr, ir.ListFilterExpr):
        ...
        try:
            for idx, item in enumerate(target):
                ctx.locals[var_name] = item
                predicate = evaluate_expression(ctx, expr.predicate, None)
    ...
    if isinstance(expr, ir.ListReduceExpr):
        ...
        acc_value = evaluate_expression(ctx, expr.start, collector)
        ...
        try:
            for idx, item in enumerate(target):
                ...
                next_value = evaluate_expression(ctx, expr.body, None)
                if collector and idx < sample_limit:
                    item_samples.append(
                        {
                            "index": idx,
    ...
    if isinstance(expr, ir.MapOpExpr):
        ...
        if expr.kind == "get":
            ...
            key = evaluate_expression(ctx, expr.key, collector)
