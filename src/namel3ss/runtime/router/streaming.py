from __future__ import annotations

from namel3ss.determinism import canonical_json_dumps


def build_sse_body(yield_messages: list[dict], response: dict) -> bytes:
    lines: list[str] = []
    for message in yield_messages:
        event_type = _safe_event_type(message.get("event_type") if isinstance(message, dict) else None)
        lines.append(f"event: {event_type}")
        lines.append(f"data: {canonical_json_dumps(message, pretty=False, drop_run_keys=False)}")
        lines.append("")
    lines.append("event: return")
    lines.append(f"data: {canonical_json_dumps(response, pretty=False, drop_run_keys=False)}")
    lines.append("")
    return "\n".join(lines).encode("utf-8")


def sorted_yield_messages(raw: object) -> list[dict]:
    if not isinstance(raw, list):
        return []
    rows: list[dict] = []
    for item in raw:
        if not isinstance(item, dict):
            continue
        sequence = _safe_int(item.get("sequence"))
        flow_name = str(item.get("flow_name") or "")
        event_type = _safe_event_type(item.get("event_type"))
        payload = {
            "flow_name": flow_name,
            "output": item.get("output"),
            "sequence": sequence,
            "event_type": event_type,
            "timestamp": _safe_timestamp(item.get("timestamp")),
        }
        if "data" in item:
            payload["data"] = item.get("data")
        if "stream_id" in item:
            payload["stream_id"] = str(item.get("stream_id") or "")
        if "stream_channel" in item:
            payload["stream_channel"] = str(item.get("stream_channel") or "")
        rows.append(payload)
    rows.sort(
        key=lambda entry: (
            int(entry.get("sequence") or 0),
            str(entry.get("flow_name") or ""),
            str(entry.get("event_type") or ""),
            canonical_json_dumps(entry.get("output"), pretty=False, drop_run_keys=False),
        )
    )
    return rows


def should_stream_response(query: dict[str, str], headers: dict[str, str], yield_messages: list[dict]) -> bool:
    explicit_request = _explicit_stream_requested(query, headers)
    if _contains_non_stream_yields(yield_messages):
        return True
    if _contains_ai_stream_events(yield_messages):
        return explicit_request
    return explicit_request


def _explicit_stream_requested(query: dict[str, str], headers: dict[str, str]) -> bool:
    stream = str(query.get("stream") or "").strip().lower()
    if stream in {"1", "true", "yes", "on"}:
        return True
    accept = str(headers.get("Accept") or headers.get("accept") or "").lower()
    if "text/event-stream" in accept:
        return True
    header_stream = str(headers.get("X-N3-Stream") or headers.get("x-n3-stream") or "").strip().lower()
    return header_stream in {"1", "true", "yes", "on"}


def _contains_non_stream_yields(yield_messages: list[dict]) -> bool:
    for item in yield_messages:
        if not isinstance(item, dict):
            return True
        if str(item.get("stream_channel") or "").strip().lower() == "ai":
            continue
        return True
    return False


def _contains_ai_stream_events(yield_messages: list[dict]) -> bool:
    for item in yield_messages:
        if not isinstance(item, dict):
            continue
        if str(item.get("stream_channel") or "").strip().lower() == "ai":
            return True
    return False


def _safe_int(value: object) -> int:
    if isinstance(value, bool):
        return 0
    try:
        parsed = int(value)
    except Exception:
        return 0
    if parsed < 0:
        return 0
    return parsed


def _safe_event_type(value: object) -> str:
    token = str(value or "").strip().lower()
    if token in {"token", "finish", "error", "progress", "yield", "return"}:
        return token
    return "yield"


def _safe_timestamp(value: object) -> str:
    if not isinstance(value, str):
        return ""
    return value.strip()


__all__ = [
    "build_sse_body",
    "should_stream_response",
    "sorted_yield_messages",
]
