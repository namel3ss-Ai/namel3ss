...
def enforce_route_permissions(
    ...
    *,
    ...
    if not authenticated:
        raise Namel3ssError(
            _auth_error_message(entry.name, auth_error),
            details={
    ...
    if requirement.roles and not any(role in roles for role in requirement.roles):
        raise Namel3ssError(
            _permission_denied_message(entry.name),
...
def _permission_denied_message(route_name: str) -> str:
    return build_guidance_message(
        what=f'Route "{route_name}" access is not permitted.',
        why="The identity does not meet the required role or permission.",
        fix="Provide an identity with the required permissions or update the route requirements.",
        example="n3 auth require get_user --role admin",
    )


def _auth_error_message(route_name: str, code: str | None) -> str:
    if code == "token_invalid":
        return build_guidance_message(
            what=f'Route "{route_name}" cannot verify the token.',
    ...
    if code == "token_expired":
        return build_guidance_message(
            what=f'Route "{route_name}" requires a valid token.',
    ...
    if code == "session_revoked":
        return build_guidance_message(
            what=f'Route "{route_name}" requires an active session.',
    ...
    return build_guidance_message(
        what=f'Route "{route_name}" requires authentication.',
...
__all__ = ["enforce_route_permissions"]
