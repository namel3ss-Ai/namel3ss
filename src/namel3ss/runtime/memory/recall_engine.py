...
from namel3ss.runtime.memory_policy.evaluation import evaluate_border_read, evaluate_phase_start
    ...
    *,
    ...
    if phase_request is not None:
        _, phase_events = _ensure_phase_for_recall(
            ...
            store_key=space_ctx.store_key_for(SPACE_SESSION),
            contract=contract,
    ...
    for space in spaces_consulted:
        ...
        if policy.short_term_max_turns > 0:
            ...
            items = [_with_space_recall(item, space=space, owner=owner) for item in items]
    ...
    current_phase = phase_registry.current(space_ctx.store_key_for(SPACE_SESSION))
...
def _with_space_recall(item: MemoryItem, *, space: str, owner: str) -> MemoryItem:
