...
- Applies to `n3 run` in run/preview/dev, including `n3 run studio`.
...
- The shell is shared by run/preview/dev; run mode controls whether production-only or Studio-instrumented manifest content is returned.
