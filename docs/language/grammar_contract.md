...
- spec, define, use, capsule, identity, app, capabilities, policy, packs, foreign, tool
- agent, ai, record, flow, job, page, ui, ui_pack

Statements:
- start, plan, review, timeline, compute, increment, attempt, in, clear, notice, require, record
- calc, let, set, if, return, ask, parallel, run agent(s), repeat, for each, match, try
- save, create, find, update, delete

UI items:
- title, text, form, table, list, chart, use ui_pack
- chat (messages, composer, thinking, citations, memory)
- tabs, tab, default, modal, drawer, button, link, section, card_group, card, row, column, divider, image

Expressions:
- literals (number, string, boolean, null)
- input, state.<path>, attribute access
- grouped expressions, call, ask, latest

## Internal surfaces
- Parser module layout, helper functions, and AST class names.
- Error phrasing and diagnostics format, so long as the same grammar and semantics remain.
- Snapshot docs under docs/grammar/current.md remain historical references.
